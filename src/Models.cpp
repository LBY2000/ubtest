...
#include <numa.h>
#include <iostream>
void Models::bulkload_train(std::vector<uint64_t> &keys,std::vector<uint64_t> &vals){   //主要为bulk_loading阶段服务的train
        //   PLR *plr=new PLR(down_epsilon-1);
           auto addr=RackMemMalloc(sizeof(PLR), PerfLevel::L0, 0);
           if(addr==nullptr){
                  std::cout << "==== error in malloc PLR ====="<<std::endl;
                  exit(0);
           }
           PLR *plr=new(addr) PLR(down_epsilon-1);
            ...
            if((!plr->add_point(p, pos)||i==keys.size()-1)||max_num_per_model>128){   //如果add_point失败，则执行下面的操作   
                if(i!=keys.size()-1){         
                  ...
                  //  plr = new PLR(down_epsilon-1);
                    ...
                    auto cs = plr->get_segment();
                    std::pair<long double, long double>  cs_param= cs.get_slope_intercept(); 
                    append_model(cs_param.first, cs_param.second, k_iter, v_iter, pos+1);
                    continue;
                }
            }else{
                max_num_per_model++;
...
void Models::bulkload_train_up(){ //down层的情况来学习上层的up_model
    ...
    //    PLR *plr=new PLR(upper_epsilon-1);
        ...
        PLR *plr=new(addr) PLR(upper_epsilon-1);
        Upper_Model tup;
...
void Models::append_model(double slope, double intercept,
    ...
    //   LeafNode *leaf = (LeafNode *)numa_alloc_onnode(sizeof(LeafNode),2);
